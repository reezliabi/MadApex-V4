...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(2, 19, 39, 255), 7.0f, 15);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(3, 5, 13, 255), 7.0f, 15);
    window->DrawList->AddText(nullptr, 18.0f, bb.Min, ImColor(153, 176, 189, 255), label);
...
bool ImGui::ButtonExit(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 colt = GetColorU32((held && hovered) ? ImGuiCol_Anill : hovered ? ImGuiCol_Anil : ImGuiCol_Anilll);
    const ImU32 col = GetColorU32(ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 0.0f);
    window->DrawList->AddText(nullptr, 18.0f, bb.Min + ImVec2(3,15), ImColor(colt), label);
...
bool ImGui::ButtonExip(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 0.0f);
    window->DrawList->AddText(nullptr, 15.0f, bb.Min - ImVec2(-2,2), ImColor(colt), label);
...
bool ImGui::Aleynaass(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ButtonExipq(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 0.0f);
    window->DrawList->AddText(nullptr, 18.0f, bb.Min - ImVec2(-2, 2), ImColor(colt), label);
...
ImVec2 pos = window->DC.CursorPos;
const ImRect total_bb(pos + ImVec2(5,0), pos + ImVec2(square_sz + 11, label_size.y + style.FramePadding.y * 2.0f));
ItemSize(total_bb, style.FramePadding.y );
...
ImColor Soluk2 = ImColor(2, 19, 39, 255);
...
window->DrawList->AddText(nullptr, 18.0f, check_bb.Min - ImVec2(103 + 148,1), Soluk, label);
window->DrawList->AddRectFilled(check_bb.Min - ImVec2(-6, 1), check_bb.Max + ImVec2(-5, 1), Soluk2, 7.0f, 15);
window->DrawList->AddRectFilled(check_bb.Min - ImVec2(-7, 0), check_bb.Max + ImVec2(-6, 0), ImColor(3,5,13,255), 7.0f, 15);
//window->DrawList->AddCircleFilled(FirstHalf.Min + ImVec2(7.3, 8.5), 8, Soluk, 100);
window->DrawList->AddRectFilled(FirstHalf.Min + ImVec2(7, 1), FirstHalf.Max - ImVec2(1,2), Soluk, 7.0f, 15);
...
window->DrawList->AddRectFilled(SecondHalf.Min + ImVec2(1, 1), SecondHalf.Max - ImVec2(7, 2), Blueg, 7.0f, 15);
}
...
bool ImGui::Checkboxes(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (pressed)
        MarkItemEdited(id);
    ImColor Blueg = ImColor(3, 168, 245, 255);
    ImColor Blueg2 = ImColor(3, 23, 46, 255);
    ImColor Siyah = ImColor(3, 5, 13, 255);
    ImColor Siyah2 = ImColor(2, 19, 39, 255);
    ImColor Renk5 = ImColor(153, 176, 189, 255);
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddCircleFilled(center, 10, Siyah2, 16);
    window->DrawList->AddCircleFilled(center, 8, Siyah, 100);

    //if (active )
    //{
    //    //const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    //    window->DrawList->AddCircleFilled(center, 10, Blueg2, 100);
    //    window->DrawList->AddCircleFilled(center, 8, Blueg, 100);
    //}
    ...
    if (label_size.x > 0.0f)
        //RenderText(ImVec2(check_bb.Max.x-120 + style.ItemInnerSpacing.x, check_bb.Min.y -2 + style.FramePadding.y), label);
...
bool ImGui::SliderScalarr(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect frame_bt(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 19 + style.FramePadding.y * 1.0f));
    const ImRect frame_bz(window->DC.CursorPos, window->DC.CursorPos + ImVec2(20, 19 + style.FramePadding.y * 1.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    const bool hovered = ItemHoverable(frame_bt, id);
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
        ...
        if (focus_requested || clicked || g.NavActivateId == id || g.NavInputId == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && (focus_requested || (clicked && g.IO.KeyShift) || g.NavInputId == id))
            {
                temp_input_is_active = true;
                FocusableItemUnregister(window);
            }
    ...
    if (temp_input_is_active)
    {
        ...
        return TempInputScalar(frame_bt, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);
    }

    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max - ImVec2(0, 7), ImColor(2, 19, 39, 255), true, g.Style.FrameRounding);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(40, 13), grab_bb.Max - ImVec2(0, 5), ImColor(1, 76, 114, 255), true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        ...
        window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(6, 12), 8, GetColorU32(ImGuiCol_SliderGrab), 100);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max - ImVec2(0, 8), ImColor(2, 19, 39, 255), true, g.Style.FrameRounding);

    window->DrawList->AddText(nullptr, 18.0f, frame_bb.Min - ImVec2(0,16), ImColor(153, 176, 189, 255), label);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 15), grab_bb.Max - ImVec2(1, 7), ImColor(1, 76, 114, 255), true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x )
       ...
       // window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(3, 12), 6, GetColorU32(ImGuiCol_SliderGrab), 100);

        window->DrawList->AddRectFilled(grab_bb.Min + ImVec2(-1, 4), grab_bb.Min + ImVec2(10, 20), Blueg, 7.0f, 15);
...
bool ImGui::SliderIntt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarr(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
